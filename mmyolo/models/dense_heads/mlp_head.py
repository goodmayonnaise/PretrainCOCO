...
class MLPHeadModule(BaseModule):
    ...
    def _init_layers(self):
        ...
        reg_out_channels = self.reg_max*4
        cls_out_channels = self.num_classes
        
        for i in range(self.num_levels):
            self.reg_preds.append(
                nn.Sequential(
                    Mlp(self.in_channels[i],
                        self.in_channels[i]*4,
                        self.in_channels[i]//2),
                    Mlp(self.in_channels[i]//2,
                        self.in_channels[i]*2,
                        self.in_channels[i]//4),
                    Mlp(self.in_channels[i]//4,
                        self.in_channels[i],
                        reg_out_channels)
                        ))
